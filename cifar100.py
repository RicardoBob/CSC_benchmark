...
def load_data():
    ...
    (x_train, y_train_fine), (x_test,
                              y_test_fine) = tf.keras.datasets.cifar100.load_data(label_mode="fine")
    #y_train = tf.concat([y_train_coarse, y_train_fine], 1)
    #y_test = tf.concat([y_test_coarse, y_test_fine], 1)
    # print(y_test_fine[10].numpy())
    ...
    return (x_train, y_train_fine), (x_test, y_test_fine), fine_labels
...
def prepare_data_mlp():
    ...
    x_test = x_test.reshape(
        x_test.shape[0], x_test.shape[1]*x_test.shape[2]*x_test.shape[2])
    return x_train, y_train, x_test, y_test, classes
